    ...
    async def generate_file_markdown(
        ...
        *,
        ...
        prompt = (
            ...
            '- CRITICAL — PRESERVE EXISTING CONTENT: Each fenced code block must contain the ENTIRE updated file content — never a snippet, method, or diff. The file will be overwritten verbatim with whatever you put inside the block. Before writing a file block, read the existing file and preserve every untouched import, class, method, and line. If you cannot output the full file, omit the block entirely.\n'
            '- Append-only files (Upgrade.php, schema dumps, route registries, enum lists, changelogs, migration tables): your new entry goes to the END. Existing entries above it must appear unchanged. Removing an entry from such a file is a regression.\n'
