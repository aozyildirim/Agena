...
export default function BRSettingsPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      } catch { /* no Azure creds yet — free-text input still works */ }
    };
    void run();
  }, []);
  ...
  return (
    <div style={{ display: 'grid', gap: 24, maxWidth: 720 }}>
      ...
      <div style={{ display: 'grid', gap: 20, padding: 20, borderRadius: 12, border: '1px solid var(--panel-border)', background: 'var(--panel)' }}>
        ...
        {autoEval && (
          <div style={{ display: 'grid', gap: 16, padding: '14px 16px', borderRadius: 10, border: '1px solid var(--panel-border-3)', background: 'var(--panel-alt)' }}>
            <div>
              <label style={labelStyle}>{t('br.settings.azureProject')}</label>
            ...
            <div>
              <label style={labelStyle}>{t('br.settings.autoEvalInterval')}</label>
              ...
              <div style={hintStyle}>{t('br.settings.autoEvalIntervalHint')}</div>
            </div>
